...
func runShareRemove(ctx context.Context, args []string) error {
	...
	err := localClient.TailfsShareRemove(ctx, args[0])
	if err == nil {
		fmt.Printf("Removed share %v\n", args[0])
	}
	return err
}
...
If you want a share to be accessed as a different user, you can use the -as flag. For example, to create the same share but with the local machine user "theuser", you would run:

	$ tailscale share add -as theuser docs /Users/me/Documents`
